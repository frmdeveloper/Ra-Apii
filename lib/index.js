...
async function RamalJadian() {
	if(isNaN(tanggal) && isNaN(bulan) && isNaN(tahun)) return `${AngkaOnly()}`
	...
	const $ = cheerio.load(link.data)
	let tanggal = $('#body').text().trim()
	let a = tanggal.replace('MAKNA TANGGAL JADIAN, PERNIKAHAN', '').replace('Karakteristik:', '\nKarakteristik : ').replace('< Hitung Kembali', '')
	return a
}
function Pantun() {
